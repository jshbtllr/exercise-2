...
public class ProcessFile {
	...
	public static LinkedHashMap <Key,Value> fillRow(String lineInput) {									//parses the line received from the file
		...
		try {
			while (true) {														
				//index = inputRow.indexOf(",", index);						//finds the instance of the delimiter
			...
			/*key = inputRow.substring(0, 3);
			value = inputRow.substring(4, inputRow.length());
			rowComponent.put(new Key(key), new Value(value));*/
