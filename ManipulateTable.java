...
public class ManipulateTable {
	...
	public static void searchCell(ArrayList <LinkedHashMap <Key, Value>> myTable) {
		...
		for (int i = 0; i < myTable.size(); i++) {
			oneRow = myTable.get(i);											//gets one LinkedHashMap
			...
			for (int j = 0; j < myTable.get(0).size(); j++) {
				...
				while ((index = oneCell.indexOf(searchChar, index)) != -1) {
					searchCounter++;
					found++;
					if (found == 1) {
					System.out.println("\nSearch Character \"" + searchChar + "\" found");
					}
						index += searchChar.length();
				}
					if (searchCounter > 0) {
					System.out.println(((searchCounter > 1) ? (searchCounter + " times") : "Once") + " at Row: " + (i+1) 
								       + " Column: " + (j+1));
				}
			}
		}
	...
	public static ArrayList <LinkedHashMap <Key, Value>> resetTable() {
		...
		for (int i = 0; i < row; i++) {
			...
			for (int j = 0; j < column; j++) {
				for (int k = 0; k < 2; k++) {
					count = 0;
					randomInput.setLength(0);

					for (count = 0; count < 3; count++) {
						Random number = new Random();
						generated = number.nextInt(93) + 33;
						randomInput.append((char) generated);
					}

					if (k == 0) {
						key = randomInput.toString();
					} else {
						value = randomInput.toString();
					}
				}

				oneRow.put(new Key(key), new Value(value));
			}
